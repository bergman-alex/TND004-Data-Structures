...
{
    std::vector<int> dummy(V.size());
    int evenCount = 0;
    long oddCount = dummy.size() - 1;
    ...
    std::reverse(dummy.begin() + evenCount, dummy.end()); // O(n)
    V = dummy;
}
...
namespace TND004
{
    ...
    {
        long size = std::distance(first, last);             // O(1)
        std::vector<int>::iterator mid = first + size / 2;  // O(1)
        ...
        {
            std::vector<int>::iterator it1 = stable_partition(V, first, mid, p);    // O(log(n))
            std::vector<int>::iterator it2 = stable_partition(V, mid, last, p);     // O(log(n))
